...
static int algo_copy()
{
    ...
    if(!hasData) {
        std::cout << "Warning: source buffer is all zeros" << std::endl;
    }
    
    memcpy(algo_in, v4l2_ir_dvp_share_buffer, 
           ALGO_WIDTH * ALGO_HEIGHT * sizeof(uint16_t));
    ...
    if(!hasData) {
        std::cout << "Warning: algo_in buffer is all zeros" << std::endl;
    }
...
static int algo_convert() {
    ...
    // printf("Input data range: min=%d, max=%d\n", min_val, max_val);
    ...
    for(int i = 0; i < ALGO_WIDTH * ALGO_HEIGHT; i++) {
        ...
        if(algo_out_float[i] < f_min) f_min = algo_out_float[i];
        if(algo_out_float[i] > f_max) f_max = algo_out_float[i];
    }
    
    // printf("Output Y range: min=%d, max=%d\n", y_min, y_max);
    // printf("Output float range: min=%f, max=%f\n", f_min, f_max);
...
void algo_exit()
{
    ...
    // 删除共享内存
    if(shmid_yuv >= 0) shmctl(shmid_yuv, IPC_RMID, NULL);
    if(shmid_float >= 0) shmctl(shmid_float, IPC_RMID, NULL);
