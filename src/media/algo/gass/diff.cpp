...
int Diff::partition(std::vector<uint16_t>& arr, int left, int right, int pivot_idx)
{
...
uint16_t Diff::quick_select(std::vector<uint16_t>& arr, int left, int right, int k)
{
    if (left == right)
        return arr[left];
