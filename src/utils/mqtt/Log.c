...
enum LOG_LEVELS Log_levels;

static int start_index = -1, next_index = 0;
...
static traceEntry* Log_pretrace()
{
    ...
    if (trace_queue_size != trace_settings.max_trace_entries)
    {
        ...
        if (start_index > trace_settings.max_trace_entries + 1 || next_index > trace_settings.max_trace_entries + 1)
        {
            start_index = -1;
            next_index = 0;
        }
...
static char* Log_formatTraceEntry(traceEntry* cur_entry)
{
    ...
    else
    {
        ...
        if (cur_entry->has_rc == 1)
            snprintf(&msg_buf[buf_pos], sizeof(msg_buf) - buf_pos, format, cur_entry->thread_id, cur_entry->depth, "", cur_entry->depth, cur_entry->name, cur_entry->line,
                     cur_entry->rc);
        else
            snprintf(&msg_buf[buf_pos], sizeof(msg_buf) - buf_pos, format, cur_entry->thread_id, cur_entry->depth, "", cur_entry->depth, cur_entry->name, cur_entry->line);
    }
