 ...
 */
static char* packet_names[] = {"RESERVED",  "CONNECT", "CONNACK",     "PUBLISH",  "PUBACK",  "PUBREC",   "PUBREL",    "PUBCOMP",
...
#if defined(OPENSSL)
    data = (net->ssl) ? SSLSocket_getdata(net->ssl, net->socket, remaining_length, &actual_len) : Socket_getdata(net->socket, remaining_length, &actual_len);
#else
...
int MQTTPacket_send(networkHandles* net, Header header, char* buffer, size_t buflen, int free)
{
    ...
    if (header.bits.type == PUBREL)
    {
        ...
        rc = MQTTPersistence_put(net->socket, buf, buf0len, 1, &buffer, &buflen, header.bits.type, msgId, 0);
    }
#endif
...
int MQTTPacket_sends(networkHandles* net, Header header, int count, char** buffers, size_t* buflens, int* frees)
{
    ...
    { /* persist PUBLISH QoS1 and Qo2 */
        ...
        int msgId = readInt(&ptraux);
        rc = MQTTPersistence_put(net->socket, buf, buf0len, count, buffers, buflens, header.bits.type, msgId, 0);
    }
#endif
#if defined(OPENSSL)
    if (net->ssl)
...
int MQTTPacket_send_publish(Publish* pack, int dup, int qos, int retained, networkHandles* net, const char* clientID)
{
    ...
    if (qos == 0)
        ...
        Log(LOG_PROTOCOL, 10, NULL, net->socket, clientID, pack->msgId, qos, retained, rc, min(20, pack->payloadlen), pack->payload);
    FUNC_EXIT_RC(rc);
    return rc;
}
