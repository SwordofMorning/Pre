        ...
        else
        {
            if (m->c->messageQueue->count > 0)
            {
                ...
                if (rc)
                    ...
                    Log(TRACE_MIN, -1, "False returned from messageArrived for client %s, message remains on queue", m->c->clientID);
            }
            if (pack)
            {
...
void MQTTClient_stop()
{
    ...
    if (running == 1 && tostop == 0)
    {
        ...
        if (handles != NULL)
        {
            ...
            {
                if (((MQTTClients*)(current->content))->c->connect_state > 0 || ((MQTTClients*)(current->content))->c->connected)
                    ++conn_count;
            }
        }
...
int MQTTClient_connectURIVersion(MQTTClient handle, MQTTClient_connectOptions* options, const char* serverURI, int MQTTVersion, START_TIME_TYPE start, long millisecsTimeout)
{
    MQTTClients* m = handle;
    int rc = SOCKET_ERROR;
...
int MQTTClient_connect(MQTTClient handle, MQTTClient_connectOptions* options)
{
    ...
    if (strncmp(options->struct_id, "MQTC", 4) != 0 ||
        (options->struct_version != 0 && options->struct_version != 1 && options->struct_version != 2 && options->struct_version != 3 && options->struct_version != 4))
    {
    ...
    if ((options->username && !UTF8_validateString(options->username)) || (options->password && !UTF8_validateString(options->password)))
    {
        rc = MQTTCLIENT_BAD_UTF8_STRING;
        goto exit;
    }
...
int MQTTClient_publishMessage(MQTTClient handle, const char* topicName, MQTTClient_message* message, MQTTClient_deliveryToken* deliveryToken)
{
    ...
    rc = MQTTClient_publish(handle, topicName, message->payloadlen, message->payload, message->qos, message->retained, deliveryToken);
exit:
    FUNC_EXIT_RC(rc);
    return rc;
}
