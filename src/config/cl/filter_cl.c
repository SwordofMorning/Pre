...
int FilterCL_ProcessMean(FilterCL* cl, uint8_t* input, uint8_t* output,
                        int width, int height, int window_size)
{
    if(!cl->initialized) return -1;
    ...
    // 执行内核
    err = clEnqueueNDRangeKernel(cl->queue, cl->kernel_mean_filter, 2, NULL,
...
void FilterCL_Cleanup(FilterCL* cl)
{
    ...
    if(cl->d_output) clReleaseMemObject(cl->d_output);
    if(cl->kernel_mean_filter) clReleaseKernel(cl->kernel_mean_filter);
